...
#include "Ifx_Math_Trigonometric_F16_Cfg.h"

/**
 * Ratio between the integer representing the value of the abscissa in Q15 format and the lookup table index.
 */
#define IFX_MATH_ATAN_INPUT_TO_INDEX (15 - IFX_MATH_TRIGONOMETRIC_F16_CFG_ATAN_LUT_SIZE)
 ...
 */
extern const uint16_t Ifx_Math_Lut_Atan_F16_table[];

/**
 *  \brief Calculates the arctangent of the input.
 *
 *  This function returns a value in the range [0, 2^29] if the input is greater
 *  than or equal to zero, where 2^29 represents \f$\pi/4\f$. If the input is less
 *  than zero the function returns a value in the range [7/8*2^32-1, 2^32-1], where
 *  7/8*2^32-1 represents \f$7*\pi/4\f$. The resolution of the output can be
 *  modified by configuring the size of the lookup table. The size of the lookup
 *  table is given by IFX_MATH_TRIGONOMETRIC_F16_CFG_ATAN_LUT_SIZE. The valid
 *  configuration set is: IFX_MATH_TRIGONOMETRIC_F16_CFG_ATAN_LUT_SIZE =
 *  {IFX_MATH_USROPT_ATAN_LUT_SIZE_8; IFX_MATH_USROPT_ATAN_LUT_SIZE_10;
 *  IFX_MATH_USROPT_ATAN_LUT_SIZE_12}.
 *
 *  \param [in] x 16-bit fractional number with the format Q15
 *
 *  \return 32-bit angle, normalized by \f$2*\pi\f$
 */
static inline uint32_t Ifx_Math_Atan_F16(Ifx_Math_Fract16 x)
{
    ...
    tableValue = ((uint32_t)Ifx_Math_Lut_Atan_F16_table[input]) << 16;
