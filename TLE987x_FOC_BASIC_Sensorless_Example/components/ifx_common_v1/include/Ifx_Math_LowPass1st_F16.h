...
/**
 * \file Ifx_Math_LowPass1st_F16.h
...
#ifndef IFX_MATH_LOWPASS1ST_F16_H
#define IFX_MATH_LOWPASS1ST_F16_H
#include "Ifx_Math.h"
 ...
 */
#define IFX_MATH_LOWPASS1ST_F16_ONE_2Q14 (16384)

/**
 * \brief Data structure that stores all data of module instance.
 */
typedef struct Ifx_Math_LowPass1st_F16
{
    /**
 ...
 */
void Ifx_Math_LowPass1st_F16_init(Ifx_Math_LowPass1st_F16* self);

/**
 *  \brief The time-discrete implementation of a 1st order low-pass filter.
 *
 *  This function provides the time-discrete implementation of a 1st order low-pass
 *  filter, described in continuous Laplace domain by the following transfer
 *  function:
 *
 *  \f[\begin{equation} \frac{G(s)}{U(s)} = H\cdot \frac{1}{\mathcal{T} \cdot (s+1)}
 *  \end{equation}\f]
 *  where U(s) is the input, G(s) is the filtered output, H is the gain, and
 *  \f$\begin{equation} \mathcal{T} \end{equation}\f$ is the time constant of the
 *  filter.
 *
 *  The time-discrete implementation of the 1st order low-pass filter is based on
 *  the continuous to discrete method \f$s \rightarrow \frac{z - 1}{z \cdot T_s}\f$,
 *  resulting in the following optimized form:
 *
 *  \f[\begin{equation} y_n = TConstCoeff \cdot (H\cdot u_n - y_{n-1})+y_{n-1}
 *  \end{equation}\f]
 *
 *  where:
 ...
 *  - \f$\begin{equation} y_{n-1} \end{equation}\f$ is the output at previous time
 *  cycle,
 *  - \f$ H \f$ is the gain of the filter,
 *  - \f$\begin{equation} TConstCoeff = \frac{T_s}{\mathcal{T}+T_s}
 *  \end{equation}\f$, where:
 *  - \f$\begin{equation} \mathcal{T} \end{equation}\f$ is the time constant of the
 *  filter.
 ...
 */
Ifx_Math_Fract16 Ifx_Math_LowPass1st_F16_execute(Ifx_Math_LowPass1st_F16* self, Ifx_Math_Fract16 input);

/**
 *  \brief Set the output of the 1st order low-pass filter to the value passed to the
 ...
 */
static inline void Ifx_Math_LowPass1st_F16_setPreviousValue(Ifx_Math_LowPass1st_F16* self, Ifx_Math_Fract16
                                                            previousValue)
{
...
/**
 *  \brief Set the time constant of the 1st order low-pass filter to the value passed to
 *  the "timeConstant" parameter.
 *
 *  \param [inout] self Reference to structure that contains instance data members
 *
 *  \param [in] timeConstant_us the time constant of the filter in microseconds, restricted to the range [0us,
 * 2^31us]
 */
static inline void Ifx_Math_LowPass1st_F16_setTimeConstant_us(Ifx_Math_LowPass1st_F16* self, uint32_t timeConstant_us)
{
...
/**
 *  \brief Set the sampling time of the 1st order low-pass filter to the value passed to
 *  the "samplingTime" parameter.
 *
 *  \param [inout] self Reference to structure that contains instance data members
 *
 *  \param [in] samplingTime_us the discrete sampling time period of the filter in microseconds, restricted to
 * the range [1us, 10^6us]
 */
static inline void Ifx_Math_LowPass1st_F16_setSamplingTime_us(Ifx_Math_LowPass1st_F16* self, uint32_t samplingTime_us)
{
    /* timeConstCoeff = samplingTime/(samplingTime+timeConstant), calculated in Q15 since tConstCoeff is between 0 and 1
     * */
 ...
 */
static inline uint32_t Ifx_Math_LowPass1st_F16_getTimeConstant_us(Ifx_Math_LowPass1st_F16* self)
{
 ...
 */
static inline uint32_t Ifx_Math_LowPass1st_F16_getSamplingTime_us(Ifx_Math_LowPass1st_F16* self)
{
 ...
 */
static inline void Ifx_Math_LowPass1st_F16_setGain(Ifx_Math_LowPass1st_F16* self, Ifx_Math_Fract16 gainQ14)
{
    /* Limit the gain between 0 an 1 */
    if (gainQ14 > IFX_MATH_LOWPASS1ST_F16_ONE_2Q14)
    {
        self->p_gainQ14 = IFX_MATH_LOWPASS1ST_F16_ONE_2Q14;
    }
    else if (gainQ14 < 0)
    {
        self->p_gainQ14 = 0;
 ...
 */
static inline Ifx_Math_Fract16 Ifx_Math_LowPass1st_F16_getGain(Ifx_Math_LowPass1st_F16* self)
{
 ...
 */
static inline Ifx_Math_Fract16 Ifx_Math_LowPass1st_F16_getPreviousValue(Ifx_Math_LowPass1st_F16* self)
{
    return (Ifx_Math_Fract16)Ifx_Math_ShR_F32(self->p_previousValue, 15);
}


#endif /*IFX_MATH_LOWPASS1ST_F16_H*/
