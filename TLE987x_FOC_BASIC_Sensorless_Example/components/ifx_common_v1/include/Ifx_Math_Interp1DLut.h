 ...
 *
 *  This function returns the interpolated values of a 1-D function at specific
 *  query points defined by a 1-D lookup table using linear interpolation.
 *  The linear interpolation is calculated according to the following equation:
 *
 *  \f[\begin{equation} y = y_1 + \frac{y_2 - y_1}{x_2 - x_1}\cdot (x-x_1)
 *  \end{equation}\f]
 *
 *  where:
 *
 *  \f$ y \f$ is the interpolated value, returned by the function
 *  \f$y_1\f$ and \f$y_2\f$ are the query points on the ordinate, given by the
 *  values of the 1-D loop up table array
 *  \f$x_1\f$ and \f$x_2\f$ are the related points on the abscissa, given by the
 *  index of the 1-D look up table multiplied by a defined coefficient
 *  \f$ x \f$ is the abscissa value input to the function given by the user
 *
 *  If the value of the abscissa is bigger than the table size, the last value of
 ...
 *
 *  \param [in] shift Coefficient defining the ratio between the resolution of the input and the
 * resolution of the abscissa of the 1-D lookup table, between 0 and 14
 *
 *  \return 16-bit interpolated ordinate value
 */
Ifx_Math_Fract16 Ifx_Math_Interp1DLut_F16(Ifx_Math_Fract16 x, Ifx_Math_Interp1DLut_Type table, uint8_t shift);
