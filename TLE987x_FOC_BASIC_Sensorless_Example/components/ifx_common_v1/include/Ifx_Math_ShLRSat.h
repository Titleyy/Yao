...
/**
 *  \brief Returns the result of the 16-bit input parameter shifted by n bits with
 *  saturation.
 *
 *  This function Returns the result of the 16-bit input parameter shifted by n bits
 *  with saturation. For a left shift, the result is limited between
 *  IFX_MATH_FRACT16_MIN and IFX_MATH_FRACT16_MAX.
 *
 *  \param [in] x 16-bit fractional number with the format Qa
 *
 *  \param [in] n Number of bits to shift the input, between -15 and 15. Positive n indicates left
 * shift, negative n indicates right shift
 *
 *  \return 16-bit result of the operation
 */
static inline Ifx_Math_Fract16 Ifx_Math_ShLRSat_F16(Ifx_Math_Fract16 x, int8_t n)
{
    /* Return value of the function */
    Ifx_Math_Fract16 shiftReturn;

    /* Negate the shift for shifting right */
    int8_t           negativeShift;

    if (n >= 0)
    {
        shiftReturn = Ifx_Math_ShLSat_F16(x, (uint8_t)(n));
    }
    else
    {
        negativeShift = -n;
        shiftReturn   = Ifx_Math_ShR_F16(x, (uint8_t)negativeShift);
    }
...
/**
 *  \brief Returns the result of the 32-bit input parameter shifted by n bits with
 *  saturation.
 *
 *  This function returns the result of the 32-bit input parameter shifted by n bits
 *  with saturation. For a left shift, the result is limited between
 *  IFX_MATH_FRACT32_MIN and IFX_MATH_FRACT32_MAX.
 *
 *  \param [in] x 32-bit fractional number with the format Qa
 *
 *  \param [in] n Number of bits to shift the input, between -31 and 31. Positive n indicates left
 * shift, negative n indicates right shift
 *
 *  \return 32-bit result of the operation
 */
static inline Ifx_Math_Fract32 Ifx_Math_ShLRSat_F32(Ifx_Math_Fract32 x, int8_t n)
{
    /* Return value of the function */
