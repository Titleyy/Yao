...
/**
 *  \brief Return the result of the 16-bit fractional input parameter shifted right by n
 *  bits without saturation or overflow detection.
 *
 ...
 * times." */
static inline Ifx_Math_Fract16 Ifx_Math_ShR_F16(Ifx_Math_Fract16 x, uint8_t n)
{
...
/**
 *  \brief Return the result of the 32-bit fractional input parameter shifted right by n
 *  bits without saturation or overflow detection.
 *
...
/* polyspace-begin CODE-METRIC:CALLING [Justified:Low] "Common math library functions are expected to be called multiple
 * times." */
static inline Ifx_Math_Fract32 Ifx_Math_ShR_F32(Ifx_Math_Fract32 x, uint8_t n)
{
    /* polyspace +2 MISRA2012:10.1 [Justified:Low] "Bitwise operators on signed values are required by fixed point
     * arithmetic." */
