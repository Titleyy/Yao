...
/**
 *  \brief Calculates the bits need to be shifted right when multiplying two fractional
 *  types.
 *
 ...
 */
static inline uint8_t Ifx_Math_MulShR_ShiftMul(Ifx_Math_FractQFormat typeX, Ifx_Math_FractQFormat typeY,
                                               Ifx_Math_FractQFormat typeReturn)
{
    uint8_t rightShift;

    if (((uint8_t)typeX + (uint8_t)typeY) >= (uint8_t)typeReturn)
    {
        rightShift = ((uint8_t)typeX + (uint8_t)typeY) - (uint8_t)typeReturn;
    }
    else
    {
        rightShift = 0u;
 ...
 *  numbers, x and y, right shifted by n, without saturation and overflow detection.
 *
 ...
 * times." */
static inline Ifx_Math_Fract16 Ifx_Math_MulShR_F16(Ifx_Math_Fract16 x, Ifx_Math_Fract16 y, uint8_t n)
{
...
/* polyspace-begin CODE-METRIC:CALLING [Justified:Low] "Common math library functions are expected to be called multiple
 * times." */
static inline Ifx_Math_Fract32 Ifx_Math_MulShR_F32(Ifx_Math_Fract32 x, Ifx_Math_Fract32 y, uint8_t n)
{
...
/* polyspace-begin CODE-METRIC:CALLING [Justified:Low] "Common math library functions are expected to be called multiple
 * times." */
static inline Ifx_Math_Fract32 Ifx_Math_MulShR_F32_F16F16(Ifx_Math_Fract16 x, Ifx_Math_Fract16 y, uint8_t n)
{
