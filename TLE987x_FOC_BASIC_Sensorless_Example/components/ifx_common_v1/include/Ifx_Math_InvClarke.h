 ...
 *
 *  The Inverse Clarke transformation function is calculating the quantities of the
 *  three-phase reference frame (\f$\begin{equation} I_u, I_v, I_w
 *  \end{equation}\f$) from the quantities of the two-axis orthogonal system
 *  (\f$\begin{equation} I_{\alpha}, I_{\beta} \end{equation}\f$), as expressed by
 *  the following equations:
 *
 *  \f[\begin{equation} I_u = I_{\alpha} \end{equation}\f]
 *
 *  \f[\begin{equation} I_v = -\frac{1}{2}I_{\alpha} + \frac{\sqrt{3}}{2}I_{\beta}
 *  \end{equation}\f]
 *
 *  \f[\begin{equation} I_w = -\frac{1}{2}I_{\alpha} - \frac{\sqrt{3}}{2}I_{\beta}
 *  \end{equation}\f]
 *
 *  where:
 ...
 *  The results \f$I_v\f$ and \f$I_w\f$ are limited between FRACT16_MIN and
 *  FRACT16_MAX
 *
 *  \param [in] stationary Data Container of the 2-phase stationary frame quantities {\f$I_{\alpha}\f$
 * Real, \f$I_{\beta}\f$ Imag}.
 *
