 ...
 */
static inline int8_t Ifx_Math_MulShLR_ShiftMul(Ifx_Math_FractQFormat typeX, Ifx_Math_FractQFormat typeY,
...
/**
 *  \brief Calculates the 16-bit shifted fractional product of two 16-bit fractional
 *  numbers.
 *
 *  This function calculates the 16-bit fractional product of two 16-bit fractional
 *  numbers, x and y, shifted by n, without saturation and overflow detection. The
 *  result is shifted right if n is greater than zero and left if n is less than
 *  zero.
 *
 ...
 *
 *  \param [in] n Number of bits to shift the multiplication result, between -15 and 30. Positive
 * n indicates right shift, negative n indicates left shift
 *
 ...
 * times." */
static inline Ifx_Math_Fract16 Ifx_Math_MulShLR_F16(Ifx_Math_Fract16 x, Ifx_Math_Fract16 y, int8_t n)
{
...
/**
 *  \brief Calculates the 32-bit shifted fractional product of two 32-bit fractional
 *  numbers.
 *
 *  This function calculates the 32-bit fractional product of two 32-bit fractional
 *  numbers, x and y, shifted by n, without saturation and overflow detection.
 *
 *  \param [in] x 32-bit fractional number with the format Qa
 *
 *  \param [in] y 32-bit fractional number with the format Qb
 *
 *  \param [in] n Number of bits to shift the multiplication result, between -31 and 62. Positive
 * n indicates right shift, negative n indicates left shift
 *
...
/* polyspace-begin CODE-METRIC:CALLING [Justified:Low] "Common math library functions are expected to be called multiple
 * times." */
static inline Ifx_Math_Fract32 Ifx_Math_MulShLR_F32(Ifx_Math_Fract32 x, Ifx_Math_Fract32 y, int8_t n)
{
...
/**
 *  \brief Calculates the 32-bit shifted fractional product of two 16-bit fractional
 *  numbers.
 *
 *  This function calculates the 32-bit fractional product of two 16-bit fractional
 *  numbers, x and y, shifted by n, without saturation and overflow detection.
 *
 *  \param [in] x 16-bit fractional number with the format Qa
 *
 *  \param [in] y 16-bit fractional number with the format Qb
 *
 *  \param [in] n Number of bits to shift the multiplication result, between -31 and 30. Positive
 * n indicates right shift, negative n indicates left shift
 *
 *  \return 32-bit fractional number with the format Q(a+b-n)
 */
static inline Ifx_Math_Fract32 Ifx_Math_MulShLR_F32_F16F16(Ifx_Math_Fract16 x, Ifx_Math_Fract16 y, int8_t n)
{
