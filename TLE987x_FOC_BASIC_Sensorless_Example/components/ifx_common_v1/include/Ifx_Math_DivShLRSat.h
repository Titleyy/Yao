 ...
 *  inputs.
 *
 *  This function calculates the 16-bit fractional quotient of the two 16-bit
 *  fractional inputs, x and y, where x is the numerator and y the denominator,
 *  shifted by n, saturated between IFX_MATH_FRACT16_MIN and IFX_MATH_FRACT16_MAX.
 *  The division is protected from division by zero by returning
 ...
 */
static inline Ifx_Math_Fract16 Ifx_Math_DivShLRSat_F16(Ifx_Math_Fract16 x, Ifx_Math_Fract16 y, int8_t n)
{
 ...
 *  inputs.
 *
 *  This function calculates the 32-bit fractional quotient of the two 32-bit
 *  fractional inputs, x and y, where x is the numerator and y the denominator,
 *  shifted by n, saturated between IFX_MATH_FRACT32_MIN and IFX_MATH_FRACT32_MAX.
 *  The division is protected from division by zero by returning
 *  IFX_MATH_FRACT32_MAX if x &gt;= 0 and IFX_MATH_FRACT32_MIN if x &lt; 0.
 *
 *  \param [in] x 32-bit fractional number with the format Qa
 *
 *  \param [in] y 32-bit fractional number with the format Qb
 *
 *  \param [in] n Number of bits to shift the division result, between -32 and 32. Positive n
 * indicates left shift, negative n indicates right shift
 *
 *  \return 32-bit fractional number with the format Q(a-b+n)
 */
static inline Ifx_Math_Fract32 Ifx_Math_DivShLRSat_F32(Ifx_Math_Fract32 x, Ifx_Math_Fract32 y, int8_t n)
{
