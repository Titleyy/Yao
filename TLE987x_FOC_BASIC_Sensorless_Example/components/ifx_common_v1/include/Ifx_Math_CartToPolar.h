...
#include "Ifx_Math.h"
#include "Ifx_Math_MotorControlMath_F16_Cfg.h"
 ...
 *
 *  This function is calculates the co-ordinates from Cartesian (Real, Imaginary)
 *  system to Polar (Amplitude, Angle) system. The angle is calculated using the
 *  Ifx_Math_Atan2 function. The amplitude is calculated based on the formula:
 *
 *  \f[\begin{equation} Amplitude = \sqrt{(real*real)+(imag*imag)}\end{equation}\f]
 *
 *  The resolution of the Amplitude table which ranges from [0,1] can be modified by
 *  configuring the size of the lookup table. The size of the lookup table is given
 *  by IFX_MATH_MOTORCONTROLMATH_F16_CFG_CART2POLAR_LUT_SIZE. The valid
 *  configuration is set as: IFX_MATH_MOTORCONTROLMATH_F16_CFG =
 *  {IFX_MATH_USROPT_CART2POLAR_LUT_SIZE_8; IFX_MATH_USROPT_CART2POLAR_LUT_SIZE_10;
