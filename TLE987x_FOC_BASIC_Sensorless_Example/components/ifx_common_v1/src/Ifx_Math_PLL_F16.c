...
#if IFX_MATH_ADVANCEDMATH_F16_CFG_PLL_DELAY_LENGTH == IFX_MATH_USROPT_PLL_DELAY_32
#define IFX_MATH_PLL_F16_SHIFT_FACTOR (5)
#elif IFX_MATH_ADVANCEDMATH_F16_CFG_PLL_DELAY_LENGTH == IFX_MATH_USROPT_PLL_DELAY_16
#define IFX_MATH_PLL_F16_SHIFT_FACTOR (4)
#elif IFX_MATH_ADVANCEDMATH_F16_CFG_PLL_DELAY_LENGTH == IFX_MATH_USROPT_PLL_DELAY_8
#define IFX_MATH_PLL_F16_SHIFT_FACTOR (3)
#elif IFX_MATH_ADVANCEDMATH_F16_CFG_PLL_DELAY_LENGTH == IFX_MATH_USROPT_PLL_DELAY_4
#define IFX_MATH_PLL_F16_SHIFT_FACTOR (2)
#elif IFX_MATH_ADVANCEDMATH_F16_CFG_PLL_DELAY_LENGTH == IFX_MATH_USROPT_PLL_DELAY_2
#define IFX_MATH_PLL_F16_SHIFT_FACTOR (1)
#endif

/* Macro to saturate the value with wrapping */
#define IFX_MATH_PLL_F16_2MAX         (-2 * IFX_MATH_FRACT16_MIN)
 ...
 */
static inline Ifx_Math_Fract16 Ifx_Math_PLL_F16_p_calcAvgRate(Ifx_Math_PLL_F16* self, uint32_t input);
 ...
 *
 */
static inline void Ifx_Math_PLL_F16_p_speedCalc(Ifx_Math_PLL_F16* self, Ifx_Math_Fract32 inputKp, Ifx_Math_Fract16
                                                speed);
 ...
 *
 *
 *  \param [in] input Input angle in uint32_t format
 *
 *  \return Converted angle in Ifx_Math_Fract16 format
 */
static inline Ifx_Math_Fract16 Ifx_Math_PLL_F16_p_convAngle(uint32_t input);
...
void Ifx_Math_PLL_F16_init(Ifx_Math_PLL_F16* self)
{
    ...
    for (int i = 0; i < IFX_MATH_ADVANCEDMATH_F16_CFG_PLL_DELAY_LENGTH; i++)
    {
        self->p_buffer[i] = 0;
    }
}
...
Ifx_Math_PLL_F16_Type Ifx_Math_PLL_F16_execute(Ifx_Math_PLL_F16* self, uint32_t input)
{
    ...
    Ifx_Math_PLL_F16_Type output;

    /* Get the wrapped the value from the previous execution */
    uint32_t              previousValue = self->p_previousValue;

    /* Subtract and convert to Q15 */
    uint32_t              deltaPhi    = input - previousValue;
    Ifx_Math_Fract16      deltaPhiQ15 = Ifx_Math_PLL_F16_p_convAngle(deltaPhi);
    ...
    Ifx_Math_Fract16      speedQ15 = Ifx_Math_PLL_F16_p_calcAvgRate(self, input);

    /* Calculate the speed from average rate and integrate */
    Ifx_Math_PLL_F16_p_speedCalc(self, inputKpQ24, speedQ15);
...
void Ifx_Math_PLL_F16_setPropGain(Ifx_Math_PLL_F16* self, Ifx_Math_Fract16Q propGain)
{
     ...
     * arithmetic." */
    samplingTimeQ30 = (Ifx_Math_Fract32)((((int64_t)self->p_samplingTime_us) << (int8_t)Ifx_Math_FractQFormat_q30) /
...
void Ifx_Math_PLL_F16_setSamplingTime_us(Ifx_Math_PLL_F16* self, uint32_t samplingTime_us)
{
    ...
    /* polyspace +3 MISRA2012:10.1 [Justified:Low] "Bitwise operators on signed values are required by fixed point
     * arithmetic." */
    samplingTimeQ30 = (Ifx_Math_Fract32)((((int64_t)samplingTime_us) << (int8_t)Ifx_Math_FractQFormat_q30) /
                                         IFX_MATH_MICROSECONDS_TO_SECONDS);
...
static inline Ifx_Math_Fract16 Ifx_Math_PLL_F16_p_calcAvgRate(Ifx_Math_PLL_F16* self, uint32_t input)
{
    uint32_t previousStep = self->p_buffer[self->p_bufferIndex];
    self->p_buffer[self->p_bufferIndex] = input;

    /* Increment buffer pointer */
    if (self->p_bufferIndex == (IFX_MATH_ADVANCEDMATH_F16_CFG_PLL_DELAY_LENGTH - 1))
    {
        self->p_bufferIndex = 0;
    }
    else
    ...
    uint32_t         rawAngle     = input - previousStep;
    Ifx_Math_Fract16 rawAngleWrap = Ifx_Math_PLL_F16_p_convAngle(rawAngle);

    /* Get the speed from the average rate of change of the input angle */
    Ifx_Math_Fract16 speed = Ifx_Math_ShR_F16(rawAngleWrap, IFX_MATH_PLL_F16_SHIFT_FACTOR);
...
/* Convert input angle to 16-bit representation */
    uint16_t         input16Bit = (uint16_t)(input >> 16);
    ...
    {
        inputMinusPiToPi = (Ifx_Math_Fract16)(Ifx_Math_Sub_F32((Ifx_Math_Fract32)input16Bit, IFX_MATH_PLL_F16_2MAX));
    }
                                                ...
                                                speed)
{
    ...
    uint32_t         anglePLL = (self->p_previousValue + (uint32_t)deltaSpeed);
