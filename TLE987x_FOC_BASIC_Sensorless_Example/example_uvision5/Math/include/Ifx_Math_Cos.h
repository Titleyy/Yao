 ...
 *
 *  This function calculates the cosinus value of the 32-bit input angle, in the
 *  range \f$[0, 2*\pi]\f$, normalized by \f$2*\pi\f$. The output resolution is
 ...
 */
static inline Ifx_Math_Fract16 Ifx_Math_Cos_F16(uint32_t x)
{
    /* Stores the cosine output */
    Ifx_Math_Fract16 cosOutput;

    /* Stores the sine input angle */
    uint32_t         sinInputAngle;
