 ...
 *
 *  The Clarke transformation function is calculating the quantities of the two-axis
 *  orthogonal system \f$I_{\alpha}\f$, and \f$I_{\beta}\f$ from the quantities of
 *  the three-phase reference frame \f$I_u\f$, \f$I_v\f$, and \f$I_w\f$, as
 ...
 *  \f$I_{\alpha}, I_{\beta},\f$ are the two-axis orthogonal reference frame
 *  quantities;
 *  \f$I_u + I_v + I_w = 0\f$
 *  The result \f$I_{\beta}\f$ is limited between IFX_MATH_FRACT16_MIN and
 *  IFX_MATH_FRACT16_MAX
 *
 *  These equations are only applicable for the case of a balanced system where the
 *  sum of the three-phase reference frame quantities is 0, which is the case of a
 *  three phase electrical machine.
 *
 *  \param [in] phase Data Container of the three phases {U, V, W}
 *
 *  \return Data Container of the 2-phase stationary frame quantities: \f$I_{\alpha}\f$ is
 * Real, \f$I_{\beta}\f$ is Imag
 */
