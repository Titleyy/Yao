...
/**
 *  \brief Convert 16-bit signed fractional type x with the format qFormat to single
...
static inline Ifx_Math_Float32 Ifx_Math_ConvSat_F16ToFlt32(Ifx_Math_Fract16 x, Ifx_Math_FractQFormat qForm)
{
    ...
    return (Ifx_Math_Float32)x / (Ifx_Math_Float32)Ifx_Math_ShL_F32((Ifx_Math_Fract32)1U, (uint8_t)qForm);
}


/**
 *  \brief Converts single precision floating point type to 16-bit fractional type with
 *  saturation.
 *
 *  This function Converts single precision floating point type to 16-bit fractional
 *  type. The result is saturated between IFX_MATH_FRACT16_MIN and
...
{
    /* polyspace +1 MISRA2012:D1.1 [Justified:Low] "Behavior verified by unit test." */
    Ifx_Math_Float32 temp = (x * (Ifx_Math_Float32)Ifx_Math_ShL_F32((Ifx_Math_Fract32)1U, (uint8_t)qForm));
...
/**
 *  \brief Convert 32-bit signed fractional type x with the format qFormat to single
 *  precision floating point type.
 *
...
{
    /* polyspace +1 MISRA2012:D1.1 [Justified:Low] "Behavior verified by unit test." */
    return (Ifx_Math_Float32)x / (Ifx_Math_Float32)(uint32_t)Ifx_Math_ShL_F32((Ifx_Math_Fract32)1U, (uint8_t)qForm);
}


/**
 *  \brief Converts single precision floating point type to fractional type, with
 *  saturation.
 *
 *  This function converts single precision floating point type to fractional type.
 *  The result is saturated between IFX_MATH_FRACT32_MIN and IFX_MATH_FRACT32_MAX.
 *
 *  \param [in] x Single precision floating point number
 *
 *  \param [in] qForm The format of the target fractional type
...
static inline Ifx_Math_Fract32 Ifx_Math_ConvSat_Flt32ToF32(Ifx_Math_Float32 x, Ifx_Math_FractQFormat qForm)
{
    ...
    uint64_t         powerOfTwo = 1ULL << (uint8_t)qForm;
