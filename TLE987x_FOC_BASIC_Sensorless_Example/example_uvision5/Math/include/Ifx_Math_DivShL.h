 ...
 */
extern const uint32_t Ifx_Math_Lut_DivShL_F32_table[];
 ...
 *  \return Shift value
 */
static inline int8_t Ifx_Math_DivShL_ShiftDiv(Ifx_Math_FractQFormat typeX, Ifx_Math_FractQFormat typeY,
                                              Ifx_Math_FractQFormat typeReturn)
{
    return -(int8_t)typeX + (int8_t)typeY + (int8_t)typeReturn;
}
 ...
 *
 *  This functions calculates the 32-bit fractional quotient of the two 32-bit
 *  fractional inputs, x and y, where x is the numerator and y the denominator,
 *  shifted by n without saturation and overflow detection. The division is
 *  protected from division by zero by returning IFX_MATH_FRACT32_MAX if x &gt;= 0
 *  and IFX_MATH_FRACT32_MIN if x &lt; 0.
 *
 *  \param [in] x 32-bit fractional number with the format Qa
 *
 *  \param [in] y 32-bit fractional number with the format Qb
 *
 *  \param [in] n Number of bits to shift the division result, between 0 and 16. Use a constant
 ...
 * this is justified." */
static inline Ifx_Math_Fract32 Ifx_Math_DivShL_p_F32_F32F32(Ifx_Math_Fract32 x, Ifx_Math_Fract32 y, uint8_t n)
{
    ...
    else
    {
        ...
        if (((uint32_t)x & Ifx_Math_Lut_DivShL_F32_table[n]) == reference)
        {
...
/**
 *  \brief Calculates the 32-bit shifted fractional quotient of two 32-bit fractional
 *  inputs.
 *
 *  This functions calculates the 32-bit fractional quotient of the two 32-bit
 *  fractional inputs x and y, where x is the numerator and y the denominator,
 ...
 *  \return 32-bit fractional number with the format Q(a-b+n)
 */
static inline Ifx_Math_Fract32 Ifx_Math_DivShL_F32(Ifx_Math_Fract32 x, Ifx_Math_Fract32 y, uint8_t n)
{
...
/**
 *  \brief Calculates the 16-bit shifted fractional quotient of two 16-bit fractional
 *  inputs.
 *
 *  This function calculates the fractional quotient between x and y, where x is the
 *  numerator and y the denominator, shifted by n without saturation and overflow
 *  detection. The division is protected from division by zero by returning
 ...
 */
static inline Ifx_Math_Fract16 Ifx_Math_DivShL_F16(Ifx_Math_Fract16 x, Ifx_Math_Fract16 y, uint8_t n)
{
    /* Stores the return value */
 ...
 *
 *  This functions calculates the 64-bit fractional quotient of the two 32-bit
 *  fractional inputs, x and y, where x is the numerator and y the denominator,
 *  shifted by n without saturation and overflow detection. The division is
 *  protected from division by zero by returning IFX_MATH_FRACT32_MAX if x &gt;= 0
 *  and IFX_MATH_FRACT32_MIN if x &lt; 0.
 ...
 */
static inline Ifx_Math_Fract64 Ifx_Math_DivShL_p_F64_F32F32(Ifx_Math_Fract32 x, Ifx_Math_Fract32 y, uint8_t n)
{
    /* Stores the return value */
